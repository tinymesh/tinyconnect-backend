-module(tinyconnect_handler_uart).
...
-define(TTYMOD, tinyconnect_tty).

%% public members

rescan() -> rescan(?SERVER).
rescan(Server) ->
   gen_server:call(Server, rescan).

discover() -> discover(?SERVER).
discover(Server) ->
   gen_server:call(Server, discover).

identify(Port) -> identify(Port, ?SERVER).
identify(Port, Server) ->
...
workers(Server) -> gen_server:call(Server, workers).

worker(Worker) -> worker(Worker, ?SERVER).
worker(Worker, Server) -> gen_server:call(Server, {worker, Worker}).
